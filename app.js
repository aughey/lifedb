...
function createNamedChildOf(collection, parentrecord, data, name, donefunc) {
   var time = new Date;
   if(parentrecord.named_children && parentrecord.named_children[name]) {
      donefunc("error: " + name + " already exists");
      return;
   }
   var tosave = {
      data: data,
            named_parents: [ { id: parentrecord._id, name: name } ],
      created_on: time
   };
...
function executeCommand(commands, index, state, returndata, donecallback) {
   ...
   } else if(command.op == 'new_named_child') {
      var name = command.name;
      getRecord(command.parent_id, function(record) {
            if(record) {
              createNamedChildOf(collection, record, null, name, function(child) {
